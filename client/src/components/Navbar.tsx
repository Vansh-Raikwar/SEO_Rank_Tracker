...
import { Search, BarChart3, History, LogOut, Menu, X, Target, Sun, Moon, ChartNoAxesColumnIcon, CreditCard, Zap } from "lucide-react";
...
export default function Navbar() {
    ...
    const navLinks = [
        { path: "/dashboard", label: "Dashboard", icon: <BarChart3 size={18} />, requiresPro: false },
        { path: "/analyze", label: "Analyze", icon: <Search size={18} />, requiresPro: false },
        { path: "/rank-tracker", label: "Rank Tracker", icon: <Target size={18} />, requiresPro: true },
        { path: "/history", label: "History", icon: <History size={18} />, requiresPro: true },
        { path: "/pricing", label: "Pricing", icon: <CreditCard size={18} />, requiresPro: false },
    ];

    return (
        <nav className="fixed top-0 w-full bg-background/70 backdrop-blur-lg z-50 print:hidden">
                ...
                <div className="flex items-center justify-between h-16">
                        ...
                        <div className="hidden md:flex items-center gap-1">
                            {navLinks.map((link) => (
                                <Link key={link.path} to={link.path} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-all ${isActive(link.path) ? "bg-accent/5 text-accent font-medium" : "text-muted-foreground hover:text-foreground hover:bg-muted/80"}`}>
                                    {link.icon}
                                    {link.label}
                                    {link.requiresPro && user?.plan === "free" && <Zap size={12} className="text-primary ml-1" />}
                                </Link>
                            ))}
                        </div>
                    )}
                    ...
                    <div className="hidden md:flex items-center gap-3">
                        ...
                        {user ? (
                            <>
                                {user.plan === "free" && (
                                    <Link to="/pricing" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary/10 border border-primary/20 text-primary text-xs font-semibold hover:bg-primary/20 transition-all">
                                        <Zap size={13} />
                                        Upgrade
                                    </Link>
                                )}
                                ...
                                {!user.isAccountVerified && (
                                    <button onClick={() => navigate("/email-verify")} className="px-4 py-1.5 rounded-lg bg-accent/10 border border-accent/20 text-accent text-xs font-semibold hover:bg-accent hover:text-accent-foreground transition-all">
                                        Verify Email
                                    </button>
                                )}
                                ...
                                <div className="py-2 space-y-1">
                                    {navLinks.map((link) => (
                                        ...
                                        >
                                            {link.icon}
                                            {link.label}
                                            {link.requiresPro && user?.plan === "free" && <Zap size={14} className="text-primary ml-auto" />}
                                        </Link>
                                    ))}
                                </div>
                                <button
