        ...
        <Link key={analysis._id} to={`/report/${analysis._id}`} className="glass rounded-2xl p-5 hover:bg-muted/50 transition-all group block">
            ...
            {analysis.status === "completed" && (
                <div className="grid grid-cols-4 gap-2">
                    {[
                        { label: "SEO", value: analysis.category?.seo ?? 0 },
                        { label: "Perf", value: analysis.category?.performance ?? 0 },
                        { label: "A11y", value: analysis.category?.accessibility ?? 0 },
                        { label: "BP", value: analysis.category?.bestPractices ?? 0 },
                    ].map((c) => (
