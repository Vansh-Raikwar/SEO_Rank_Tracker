...
import { Target, Plus, RefreshCw, Trash2, TrendingUp, TrendingDown, Minus, ExternalLink, Clock, Loader2, X, Search, Globe, AlertCircle, Eye, EyeOff, Filter, ArrowUpDown, Zap } from "lucide-react";
...
export default function RankTracker() {
    ...
    const handleDelete = async (id: string) => {
        ...
        setDeleting(id);
        try {
            const res = await api.delete(`/api/rank/${id}/delete`);
            if (res.data.success) {
    ...
    if (!isPro) {
        return (
            <div className="min-h-screen pt-16 md:pt-24 bg-background flex items-center justify-center">
                <div className="max-w-md mx-auto px-4 py-12 text-center">
                    ...
                    <p className="text-muted-foreground mb-8">
                        Historical rank tracking and competitor analysis are only available on the Pro plan. Upgrade to unlock unlimited keyword tracking.
                    </p>
                    <Link to="/pricing" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-primary text-sm font-semibold text-primary-foreground hover:opacity-90 transition-opacity" style={{ color: "var(--background)" }}>
                    ...
                    <div className="bg-background border border-border rounded-2xl p-6 w-full max-w-md">
                        ...
                        <form onSubmit={handleAdd} className="space-y-4">
                            ...
                            <div className="bg-primary/5 border border-primary/20 rounded-xl p-3 text-xs text-muted-foreground">
                                <p>💡 We'll search Google for your keyword, find your website's position (up to 50 pages), and track it daily.</p>
                            </div>
