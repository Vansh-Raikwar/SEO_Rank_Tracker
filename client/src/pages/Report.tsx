...
import { ArrowLeft, Globe, Clock, FileText, Image, Link2, Heading, Tag, AlertCircle, ExternalLink, Type, Search, Download, Mail, Zap, Loader2 } from "lucide-react";
...
export default function Report() {
    ...
    const handleEmailReport = async () => {
        ...
        setError("");
        try {
            const { data } = await api.post(`/api/analysis/${id}/email`);
            if (data.success) {
    ...
    const handleExportPdf = async () => {
        ...
        window.open(`http://localhost:8080/api/analysis/${id}/pdf?token=${token}`, '_blank');
    };
    ...
    return (
        <div className="min-h-screen pt-16 md:pt-24 bg-background">
            <div id="report-content" className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
                {/* Back + Header */}
                <div className="mb-8">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                        ...
                        {/* Action Buttons */}
                        <div className="flex flex-wrap items-center gap-3 shrink-0 print:hidden mt-4 sm:mt-0">
                            {emailSuccess && <span className="text-emerald-500 text-xs font-medium mr-2">{emailSuccess}</span>}
                            
                            {isPro ? (
                                <button onClick={handleEmailReport} disabled={sendingEmail} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-muted text-foreground text-sm font-semibold hover:bg-muted/80 transition-colors border border-border disabled:opacity-50">
                                    {sendingEmail ? <Loader2 size={16} className="animate-spin" /> : <Mail size={16} />}
                                    Email Report
                                </button>
                            ) : (
                                <Link to="/pricing" className="flex items-center gap-2 px-4 py-2 rounded-xl bg-primary/10 text-primary text-sm font-semibold hover:bg-primary/20 transition-colors border border-primary/20">
                                    <Zap size={16} />
                                    Email Report
                                </Link>
                            )}
                            ...
                            <button 
                                ...
                                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:opacity-90 transition-opacity" style={{ color: "var(--background)" }}>
                                <Download size={16} />
                                Export PDF
                            </button>
                        </div>
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-8">
                        ...
                        <div className="flex-1 w-full">
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                {[
                                    { label: "SEO", value: analysis.category?.seo ?? 0, icon: <Search size={18} /> },
                                    { label: "Performance", value: analysis.category?.performance ?? 0, icon: <Clock size={18} /> },
                                    { label: "Accessibility", value: analysis.category?.accessibility ?? 0, icon: <Globe size={18} /> },
                                    { label: "Best Practices", value: analysis.category?.bestPractices ?? 0, icon: <Tag size={18} /> },
                                ].map((cat) => (
                    ...
                    {activeTab === "meta" && (
                        <div className="bg-card border border-border rounded-2xl p-6 shadow-sm">
                            ...
                            <div className="space-y-4">
                                {[
                                    { label: "Title", value: analysis.metaData?.title || "", ideal: "50-60 characters", len: (analysis.metaData?.title || "").length, tip: "Add a descriptive page title for better SEO ranking." },
                                    { label: "Description", value: analysis.metaData?.description || "", ideal: "150-160 characters", len: (analysis.metaData?.description || "").length, tip: "Write a compelling meta description to improve click-through rates." },
                                    { label: "Canonical URL", value: analysis.metaData?.canonical || "", tip: "Set a canonical URL to prevent duplicate content issues." },
                                    { label: "Robots", value: analysis.metaData?.robots || "", tip: "Add a robots meta tag to control search engine crawling behavior." },
                                    { label: "Viewport", value: analysis.metaData?.viewport || "", tip: "Add a viewport meta tag for proper mobile responsiveness." },
                                    { label: "Charset", value: analysis.metaData?.charset || "", tip: "Declare character encoding (e.g., UTF-8) for proper text rendering." },
                                    { label: "OG Title", value: analysis.metaData?.ogTitle || "", tip: "Add an Open Graph title for better social media sharing previews." },
                                    { label: "OG Description", value: analysis.metaData?.ogDescription || "", tip: "Add an OG description for richer social media link previews." },
                                    { label: "OG Image", value: analysis.metaData?.ogImage || "", tip: "Add an OG image (1200×630px recommended) for social media thumbnails." },
                                    { label: "Twitter Card", value: analysis.metaData?.twitterCard || "", tip: "Add a Twitter Card meta tag (summary_large_image recommended)." },
                                ].map((meta) => (
                                    <div key={meta.label} className="bg-muted/50 border border-border rounded-xl p-4">
                                        ...
                                        {meta.value ? (
                                            ...
                                            <div>
                                                <p className="text-sm text-danger/80 font-medium">Not found</p>
                                                <p className="text-xs text-muted-foreground mt-1">💡 {meta.tip}</p>
                                            </div>
                                        )}
