...
export default function RankDetail() {
    ...
    const handleRefresh = async () => {
        ...
        setRefreshing(true);
        try {
            const { data } = await api.post(`/api/rank/${id}/refresh`);
            if (data.success) {
                toast.success("Rank check started!");
                setTracking(data.tracking);
            }
        } catch (error: any) {
            toast.error(error.response?.data?.message || "Failed to refresh rank");
            setRefreshing(false);
        }
    };
    ...
    // Polling effect when status is "checking"
    useEffect(() => {
        let interval: any;
        if (tracking?.status === "checking") {
            interval = setInterval(async () => {
                try {
                    ...
                    if (data.success) {
                        setTracking(data.tracking);
                        if (data.tracking.status !== "checking") {
    ...
    const drawChart = () => {
        ...
        const labelCount = Math.min(5, maxPos - minPos + 1);
        const labels = [];
        for (let i = 0; i < labelCount; i++) {
            labels.push(Math.round(minPos + (maxPos - minPos) * (i / (labelCount - 1))));
        }
        const uniqueLabels = [...new Set(labels)].sort((a, b) => a - b);

        uniqueLabels.forEach((posVal) => {
            const yNorm = (posVal - minPos) / (maxPos - minPos);
        ...
        // Check if all points are from the same day
        const allSameDay = history.length > 1 && history.every(h => new Date(h.date).toDateString() === new Date(history[0].date).toDateString());
